...
import random
class Kmean:
    def __init__(self, k, max_iter=10, initiation= "Forgy", num_centroids= 4, distance = "Euclidean"):
        self.k = k
        self.k_num = k.shape[0] # number of centroids
        self.dimension = k.shape[1] # dimension of the data
        self.particles = [Particle(k[i], index = i) for i in range(self.k_num)]
        ...
        self.membership = [[ 0 for j in range(num_centroids)] for i in range(self.k_num)] # membership matrix
    
    def initiate(self) -> None:
        # initiate the centroids
        if self.initiation == "Forgy":
            # randomly select k data points as centroids
            self.clusters = [Cluster(Centroid(self.k[random.randint(0, self.k_num-1)], i))for i in range(self.num_centroids)]
    ...
    def update_centroids(self):
        ...
        for i in range(self.num_centroids):
            ...
            data_points = [self.particles[j].loc for j in range(self.k_num) if self.membership[j][i] == 1]
            ...
            self.clusters[i].particles = [self.particles for j in range(self.k_num) if self.membership[j][i] == 1]
    ...
    def label(self) ->list:
        ...
        return [np.argmax(self.membership[i]) for i in range(self.k_num)]
